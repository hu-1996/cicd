  ...
  return (
    <>
      {steps?.map((step: any, index: number) => (
        <div>
          <div key={step.last_runner_id} className="flex items-center float-right">
            <Tooltip
              placement="leftBottom"
              title={step.name}
              key={step.last_runner_id}
            >
              <div
            ...
            (steps[index + 1].last_status === "pending" ||
              steps[index + 1].last_status === "") ? (
              <Button
                disabled={
                  steps[index].last_status !== "success" &&
                  steps[index].last_status !== "failed"
                }
                type="text"
                size="small"
                className="h-[16px]"
                onClick={() => startNextStep(steps[index + 1].last_runner_id)}
              >
